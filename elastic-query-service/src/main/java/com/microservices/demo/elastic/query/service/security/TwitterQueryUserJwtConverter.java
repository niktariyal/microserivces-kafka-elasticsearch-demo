...
import com.microservices.demo.elastic.query.service.constant.Constants;
...
public class TwitterQueryUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public static final Logger LOG = LoggerFactory.getLogger(TwitterQueryUserJwtConverter.class);
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        ...
        return Optional.ofNullable(twitterQueryUserDetailsService
                .loadUserByUsername(jwt.getClaimAsString(USERNAME_CLAIM)))
                .map(userDetails -> {
    ...
    private Collection<String> getRoles(Jwt jwt){
        Object roles = ((Map<String,Object>)jwt.getClaims().get(REALM_ACCESS_CLAIM))
                .get(ROLES_CLAIM);
        if(roles instanceof Collection){
            return ((Collection<String>) roles).stream()
                    .map(authority -> DEFAULT_ROLE_PREFIX+authority.toUpperCase(Locale.US))
                    .collect(Collectors.toList());
        }
    ...
    private Collection<String> getScopes(Jwt jwt){
        ...
        if (scopes instanceof String) {
            return Arrays.stream(((String)scopes).split(SCOPE_SEPARATOR))
                    .map(authority -> DEFAULT_SCOPE_PREFIX+authority.toLowerCase(Locale.US))
                    .collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
