...
public class TwitterElasticRepositoryQueryClient implements ElasticQueryClient<TwitterIndexModel> {
    ...
    public List<TwitterIndexModel> getAllIndexModels() {
        List<TwitterIndexModel> searchResult =
                CollectionUtil.getInstance().getListFromIterable(twitterElasticsearchQueryRepository.findAll());
        LOG.info("{} number of documents retrieved successfully", searchResult.size());
        return searchResult;
    }

    @Override
    public void deleteIndexModelById(String id) {
        twitterElasticsearchQueryRepository.deleteById(id);
        LOG.info("Document with id {} deleted successfully", id);
    }

    @Override
    public List<TwitterIndexModel> deleteIndexModelsByText(String text) {
        return null;
    }
}
